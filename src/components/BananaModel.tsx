...
const Banana = ({ z }: { z: number }) => {
  ...
  useFrame(() => {
    ...
    bananaRef.current.position.set(data.x, data.y += 0.005, z);
...
export default function BananaModel({ count, depth }: { count: number, depth: number }) {
  return (
    <div className="w-full h-screen flex items-center justify-center">
      <Canvas gl={{ alpha: false }} camera={{ near: 0.1, far: 110, fov: 80 }}>
